...
public class Puzzle02 {
    ...
    public static int solvePart1(String[] passwordList) {
        int numberOfCorrectPasswords = 0;
        Pattern passwordPattern = Pattern.compile("(\\d+)-(\\d+) (.): (.+)");
        for (String password : passwordList) {
            Matcher passwordMatcher = passwordPattern.matcher(password);
            if (!passwordMatcher.matches()) {
                System.out.println("Oopsy...");
            } else {
                int minimum = Integer.parseInt(passwordMatcher.group(1));
                int maximum = Integer.parseInt(passwordMatcher.group(2));
                char charecter = passwordMatcher.group(3).charAt(0);
                int appearences = 0;
                for (char c : passwordMatcher.group(4).toCharArray()) {
                    if (c == charecter) {
                        appearences++;
                    }
                }
                if (appearences >= minimum && appearences <= maximum) {
    ...
    public static int solvePart2(String[] passwordList) {
        int numberOfCorrectPasswords = 0;
        Pattern passwordPattern = Pattern.compile("(\\d+)-(\\d+) (.): (.+)");
        for (String password : passwordList) {
            Matcher passwordMatcher = passwordPattern.matcher(password);
            if (!passwordMatcher.matches()) {
                ...
                int minimum = Integer.parseInt(passwordMatcher.group(1)) - 1;
                int maximum = Integer.parseInt(passwordMatcher.group(2)) - 1;
                char charecter = passwordMatcher.group(3).charAt(0);
                if (passwordMatcher.group(4).charAt(minimum) == charecter ^ passwordMatcher.group(4).charAt(maximum) == charecter) {
                    numberOfCorrectPasswords += 1;
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.printf("Answer part 1: %d\n", solvePart1(passwordList));
        System.out.printf("Answer part 2: %d\n", solvePart2(passwordList));
    }

    private static String[] passwordList = {"8-11 l: qllllqllklhlvtl",
            "1-3 m: wmmmmmttm",
            "2-4 p: pgppp",
