...
class Day14 extends Puzzle[Long] {
  ...
  private def createCaveSystem(scan: Array[Array[Char]], coordinates: List[List[(Int, Int)]]): Unit =
    coordinates.foreach(cs => cs.zip(cs.tail).foreach((s,f) =>
      ...
      if (deltaX == 0)
        (s._2 to f._2 by Math.signum(f._2 - s._2).toInt).foreach(y => scan(y)(s._1) = '#')
      else
        (s._1 to f._1 by deltaX).foreach(x => scan(s._2)(x) = '#')
    ))
  ...
  private def pourSand(scan: Array[Array[Char]], startX: Int, foundSolution: (Int, Int) => Boolean): Unit =
    ...
    while (!foundSolution(y, x))
      while (y + 1 < scan.length && scan(y + 1)(x) == '.')
        y += 1
      if (y + 1< scan.length && scan(y + 1)(x - 1) == '.')
        y += 1
        x -= 1
      else if (y + 1< scan.length && scan(y + 1)(x + 1) == '.')
        y += 1
        x += 1
  ...
  override def solvePart2(lines: List[String]): Long = {
    ...
    val deltaX = 1000
    val scan = Array.fill[Char](maxY + 1, deltaX + 1)('.')
    createCaveSystem(scan, List(List((0, maxY - 1), (1000, maxY - 1))) ++ coordinates)
    pourSand(scan, 500, (y, x) => scan(y)(x) == 'o')
    scan.map(_.count(_ == 'o')).sum
  }
